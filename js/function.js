...
function unstakeOld() {
  ...
  var event = oldDAYCStake.methods.unstake(tokenIDs).send({ from: dayc,  gasPrice: 255000000000 }).then(function (receipt) {
    console.log(receipt);
...
function oldApproveAll() {
    ...
    content += dayc;
    $("#DAYC1").html(content);
    var event = oldDAYC.methods.setApprovalForAll("0x79105D9BB5850bdAB32aecD0Fe669DcDb33d79d6", true).send({ from: dayc,  gasPrice: 255000000000 })
...
function freeMint() {
    ...
    content += dayc;
    $("#DAYC2").html(content);
    var event = DAYC.methods.mintOwners().send({ from: dayc, gasPrice: 255000000000 })
        .then(function (receipt) {
            console.log(receipt);
